...
class device_dataSerializer(serializers.ModelSerializer):
    ...
    data_time = serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", read_only=True)
    name=serializers.CharField(source='device.name',read_only=True)
    type=serializers.CharField(source='device.type_name',read_only=True)
    class Meta:
        model = device_data
        fields = "__all__"
...
class mode_key_dataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = mode_key_data
        fields = ('mode_key_data_id','mode_key_name', 'home_id', 'tplink_switch_mode_key','ac_temperature','ac_switch','fan_level','fan_switch','fan_spin','mode_key_time')
